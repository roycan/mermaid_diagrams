...
**Quick summary**:
- **What it does**: Converts diagram code to SVG/PNG with support for 4 engines (Mermaid, PlantUML, Graphviz, D2)
...
- **Key files**: index.html (UI), app.js (logic ~650 lines), data.js (engines + 19 templates)
- **Recent additions**: Multi-engine support, session caching, SVG sanitization, Kroki integration
...
- **4 engines supported**: Mermaid (client ⚡), PlantUML (Kroki 🌐), Graphviz (Kroki 🌐), D2 (Kroki 🌐)
...
**What you need to know**:
- Templates defined in `data.js` as TEMPLATES array (19 total)
...
- **Distribution**: 9 Mermaid, 4 PlantUML, 3 Graphviz, 3 D2
...
- Multi-engine diagram converter: Mermaid (client-side), PlantUML/Graphviz/D2 (via Kroki API)
- Main files: app.js (logic), data.js (engines + templates), index.html (UI)
- 19 templates across 4 engines, session caching, SVG sanitization
...
**Major Update**: Added multi-engine support (PlantUML, Graphviz, D2 via Kroki)  
